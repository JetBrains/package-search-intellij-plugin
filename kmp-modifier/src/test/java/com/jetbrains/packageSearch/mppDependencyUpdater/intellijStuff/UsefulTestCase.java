package com.jetbrains.packageSearch.mppDependencyUpdater.intellijStuff;// Copyright 2000-2023 JetBrains s.r.o. and contributors. Use of this source code is governed by the Apache 2.0 license.
...
public abstract class UsefulTestCase extends TestCase {
  ...
  public static void assertOrderedEquals(byte @NotNull [] actual, byte @NotNull [] expected) {
    ...
    for (int i = 0; i < actual.length; i++) {
      TestCase.assertEquals("not equals at index: " + i, expected[i], actual[i]);
    }
  }
  ...
  public static void assertSize(int expectedSize, Object @NotNull [] array) {
    if (array.length != expectedSize) {
      TestCase.assertEquals(toString(Arrays.asList(array)), expectedSize, array.length);
    }
  }

  public static void assertSize(int expectedSize, @NotNull Collection<?> c) {
    if (c.size() != expectedSize) {
      TestCase.assertEquals(toString(c), expectedSize, c.size());
    }
  }
  ...
  public static void assertThrows(@NotNull Class<? extends Throwable> exceptionClass,
    ...
    try {
      ...
      if (expectedErrorMsgPart != null) {
        TestCase.assertTrue(cause.getClass()+" message was expected to contain '"+expectedErrorMsgPart+"', but got: '"+cause.getMessage()+"'", cause.getMessage().contains(expectedErrorMsgPart));
      }
    }
    finally {
      if (!wasThrown) {
        TestCase.fail("'" + exceptionClass.getName() + "' must have been thrown, but the computation completed successfully instead");
      }
    }
  }
  ...
  protected static <T extends Throwable> void assertNoException(@NotNull Class<? extends Throwable> exceptionClass,
    ...
    try {
      ...
      if (exceptionClass.equals(cause.getClass())) {
        ...
        TestCase.fail("Exception isn't expected here. Exception message: " + cause.getMessage());
      }
      else {
        throw e;
      }
  ...
  protected void assertNoThrowable(@NotNull Runnable closure) {
    ...
    TestCase.assertNull(throwableName);
  }
