...
public abstract class GradleImportingTestCase extends JavaExternalSystemImportingTestCase {
  ...
  private static final String SUPPORTED_GRADLE_VERSIONS = "8.0";
  ...
  private void cleanScriptsCacheIfNeeded() {
    if (SystemInfo.isWindows && isGradleOlderThan("3.5")) {
      ...
      if (FileUtil.delete(scriptsCacheFolder)) {
        UsefulTestCase.LOG.debug("Gradle scripts cache folder has been successfully removed at " + scriptsCacheFolder.getPath());
      }
      else {
        UsefulTestCase.LOG.debug("Gradle scripts cache folder has not been removed at " + scriptsCacheFolder.getPath());
      }
      ...
      if (FileUtil.delete(scriptsRemappedCacheFolder)) {
        UsefulTestCase.LOG.debug("Gradle scripts-remapped cache folder has been successfully removed at " + scriptsRemappedCacheFolder.getPath());
      }
      else {
        UsefulTestCase.LOG.debug("Gradle scripts-remapped cache folder has not been removed at " + scriptsRemappedCacheFolder.getPath());
      }
    }
  }
  ...
  public static @NotNull String requireJdkHome(@NotNull GradleVersion gradleVersion) {
    ...
    TestCase.fail("Cannot find JDK for Gradle " + gradleVersion.getVersion() + ", checked paths: " + paths);
    return null;
  }
  ...
  protected void importProject(@NonNls @Language("Groovy") String config, Boolean skipIndexing) throws IOException {
    ...
    if (isGradleNewerOrSameAs("7.0")) {
      ...
      if (StringUtil.isEmpty(originalVmOptions)) {
        ...
        GradleSystemSettings.getInstance().setGradleVmOptions(String.format(originalVmOptions, failOnWarning));
      }
    }
